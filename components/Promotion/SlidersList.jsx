...
import { columnsSlider } from './_dataTable'

import { RiDeleteRow, RiFileCopy2Line, RiSearchLine } from 'react-icons/ri'
import SliderFormDrawer from './SliderFormDrawer'
import { getSliders } from '@/lib/services/marketing'
import { deleteSlider, getCollections } from '../../lib/services/marketing'
...
function SlidersList() {
  ...
  useEffect(() => {
    const unsubscribe = getSliders(setSliders)
    const unsubscribeCollection = getCollections(setCollections)
    ...
    <div className="">
      <ConfirmModal
        confirmFunction={async () => {
          deleteSlider(selectedSlider.id, selectedSlider.imageUrl)
          setOpenWarning(false)
        }}
        cancelFuction={() => {}}
        title="Suppression Sponsoring"
        description={
          "Etes vous sur de supprimer cette promotion ? L'action est irreversible"
        }
        open={openWarning}
            ...
            <div className="overflow-hidden ring-1 ring-black ring-opacity-5 md:rounded-sm">
              <table className="min-w-full table-auto divide-y divide-gray-300 text-left">
                <thead className="bg-gray-50">
                  <tr>
                    {columnsSlider.map((column, index) => (
                      <th
                        key={index}
                        scope="col"
