...
import { columnsSponsor } from './_dataTable'

import { RiDeleteRow, RiFileCopy2Line, RiSearchLine } from 'react-icons/ri'
import SponsorFormDrawer from './SponsorFormDrawer'
import ConfirmModal from '../ConfirmModal'
import { getSponsors } from '@/lib/services/marketing'
import { deleteSponsor } from '../../lib/services/marketing'
...
function SponsoringList() {
  ...
  useEffect(() => {
    const unsubscribe = getSponsors(setSponsors)
    return () => {
      unsubscribe()
    }
  }, [])
...
function SponsorsTable({ selectedSponsor, setSelectedSponsor, sponsors }) {
  ...
  return (
    <div className="">
      <ConfirmModal
        ...
        cancelFuction={() => {}}
        title="Suppression Slider"
        description={
          "Etes vous sur de supprimer ce slider ? L'action est irreversible"
        }
        open={openWarning}
            ...
            <div className="overflow-hidden ring-1 ring-black ring-opacity-5 md:rounded-sm">
              <table className="min-w-full text-left divide-y divide-gray-300 table-auto">
                <thead className="bg-gray-50">
                  <tr>
                    {columnsSponsor.map((column, index) => (
                      <th
                        key={index}
                        scope="col"
