...
import { commercialConstructorCreate, sliderConstructorCreate, sponsorConstructorCreate, collectionConstructorCreate } from '../../utils/functionFactory'
...
export const commercialsCollectionRef = collection(db, `commercials`)

export const commercialDocRef = (commercialId) => doc(db, `commercials/${commercialId}`)

export const getCommercials = (setState) => {
...
export const editCommercial = async (commercialId, data, updateImage, oldImageUrl, oldImageUrl1000) => {
  const imageData = {}
  if (updateImage) {
    const imageUrl = await getDefaultImageDownloadURL(data.imageUrl[0], `marketing/commercials`);
    const imageHash = await encodeImageToBlurhash(imageUrl)
    deleteResizedStorageImage(oldImageUrl, '1000x1000')
    deleteResizedStorageImage(oldImageUrl, '200x200')
  ...
  await updateDoc(commercialDocRef(commercialId), commercialConstructorCreate({ ...data, imageUrl: oldImageUrl, imageUrl1000: oldImageUrl1000, ...imageData }, true))
}
...
export const deleteCommercial = async (commercialId, imageUrl) => {
  deleteResizedStorageImage(imageUrl, '1000x1000')
  deleteResizedStorageImage(imageUrl, '200x200')
